...
const { AnanthapuramuRichText } = require("../../../models/ananthapuramuSchema"); // Import the model
...
router.post("/createOrUpdate", async (req, res) => {
  try {
    ...
    let richTextContent = await AnanthapuramuRichText.findOne();
    if (richTextContent) {
    ...
    richTextContent = new AnanthapuramuRichText({ title, content });
    await richTextContent.save();
...
router.get("/", async (req, res) => {
  try {
    const richTextContent = await AnanthapuramuRichText.findOne();
    if (!richTextContent) {

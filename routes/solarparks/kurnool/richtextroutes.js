...
const { KurnoolRichText } = require("../../../models/kurnoolSchema"); // Import the model
...
router.post("/createOrUpdate", async (req, res) => {
  try {
    ...
    let richTextContent = await KurnoolRichText.findOne();
    if (richTextContent) {
    ...
    richTextContent = new KurnoolRichText({ title, content });
    await richTextContent.save();
...
router.get("/", async (req, res) => {
  try {
    const richTextContent = await KurnoolRichText.findOne();
    if (!richTextContent) {

...
const { GaaliveeduRichText } = require("../../../models/gaaliveeduSchema"); // Import the model
...
router.post("/createOrUpdate", async (req, res) => {
  try {
    ...
    let richTextContent = await GaaliveeduRichText.findOne();
    if (richTextContent) {
    ...
    richTextContent = new GaaliveeduRichText({ title, content });
    await richTextContent.save();
...
router.get("/", async (req, res) => {
  try {
    const richTextContent = await GaaliveeduRichText.findOne();
    if (!richTextContent) {

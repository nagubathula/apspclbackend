...
const { KadapaRichText } = require("../../../models/kadapaSchema"); // Import the model
...
router.post("/createOrUpdate", async (req, res) => {
  try {
    ...
    let richTextContent = await KadapaRichText.findOne();
    if (richTextContent) {
    ...
    richTextContent = new KadapaRichText({ title, content });
    await richTextContent.save();
...
router.get("/", async (req, res) => {
  try {
    const richTextContent = await KadapaRichText.findOne();
    if (!richTextContent) {

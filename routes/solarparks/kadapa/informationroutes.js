...
const { KadapaInformation } = require("../../../models/npkuntaSchema");
...
const uploadFile = async (req, res) => {
  try {
    ...
    const information = new KadapaInformation({
      title,
      description,
      path: relativePath,
    });
...
const getInformations = async (req, res) => {
  try {
    ...
    const informations = await KadapaInformation.find()
      .skip((page - 1) * limit)
...
const deleteInformation = async (req, res) => {
  try {
    const { id } = req.params;
    const information = await KadapaInformation.findByIdAndDelete(id);
...
const updateInformation = async (req, res) => {
  try {
    const { id } = req.params;
    const { title, description } = req.body;
    const information = await KadapaInformation.findById(id);
...
router.put(
  "/npkuntainformations/informations/:id",
  upload.single("file"),
  updateInformation
);
